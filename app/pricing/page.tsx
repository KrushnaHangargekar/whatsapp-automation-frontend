...
export default function PricingPage() {
  ...
  // Approximate pricing calculation for Meta conversation rates
    ...
    <div className="pt-32 pb-24 space-y-20">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-5">
        ...
        <h1 className="text-4xl sm:text-6xl font-black text-slate-900 dark:text-white tracking-tight">
          Simple, Transparent <span className="gradient-text-wa">Wabtic Pricing</span>
        </h1>
        ...
        <div className="flex flex-wrap items-center justify-center gap-6 text-xs font-bold text-slate-600 dark:text-slate-400 pt-2">
          <div className="flex items-center gap-1.5 bg-white dark:bg-[#202c33] px-3.5 py-1.5 rounded-full border border-slate-200 dark:border-slate-800 shadow-sm">
        ...
        <div className="pt-6 flex flex-col sm:flex-row items-center justify-center gap-3">
          <div className="flex items-center gap-2 p-1 bg-slate-200/80 dark:bg-[#202c33] rounded-full border border-slate-300 dark:border-slate-800">
            <button
              onClick={() => setAnnual(false)}
              className={`px-5 py-2 rounded-full text-xs font-extrabold transition-all ${
                !annual ? 'bg-white dark:bg-[#111b21] text-slate-900 dark:text-white shadow-md' : 'text-slate-500 hover:text-slate-900 dark:hover:text-white'
              }`}
            >
              Monthly Billing
            </button>
            <button
              ...
              className={`px-5 py-2 rounded-full text-xs font-extrabold transition-all flex items-center gap-1.5 ${
                annual ? 'bg-[#25D366] text-[#0b141a] shadow-md font-black' : 'text-slate-500 hover:text-slate-900 dark:hover:text-white'
              }`}
            >
              Annual Billing
              <span className="w-2 h-2 rounded-full bg-[#0b141a] animate-ping" />
            </button>
          </div>

          <span className="px-3 py-1 rounded-full wa-yellow-badge text-xs shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
          {INITIAL_PRICING_PLANS.map((plan) => {
            return (
              <div
                key={plan.id}
                className={`glass-card p-7 sm:p-8 rounded-3xl border flex flex-col justify-between transition-all duration-300 relative ${
                  plan.highlight
                    ? 'border-[#25D366] shadow-2xl shadow-[#25D366]/20 ring-2 ring-[#25D366]/40 dark:bg-[#202c33] lg:-translate-y-2 z-10'
                    : 'border-slate-200 dark:border-slate-800 hover:border-[#25D366]/50'
                }`}
              >
                {plan.highlight && (
                  <div className="absolute -top-3.5 right-6 px-4 py-1 rounded-full bg-[#25D366] text-[#0b141a] text-xs font-black uppercase tracking-wider shadow-md">
                    Best Value
                  </div>
                )}
                ...
                <div>
                  ...
                  {/* Channel & User info pill */}
                  <div className="p-3.5 rounded-2xl bg-slate-100 dark:bg-[#111b21] border border-slate-200 dark:border-slate-800 mb-6 text-xs space-y-1">
                ...
                <Link
                  href="/demo"
                  className={`w-full py-4 rounded-xl font-black text-sm text-center transition-all flex items-center justify-center gap-2 border-2 ${
                    plan.highlight
                      ? 'bg-[#25D366] border-[#25D366] text-[#0b141a] shadow-lg shadow-[#25D366]/25 hover:bg-[#20bd5a]'
      ...
      <section className="max-w-4xl mx-auto px-4">
        <div className="glass-card rounded-3xl p-8 sm:p-10 border border-slate-200 dark:border-slate-800 space-y-6 shadow-xl">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-2xl bg-[#00c875]/15 text-[#00c875] flex items-center justify-center font-bold">
          ...
          <div className="space-y-4 pt-2">
            <div className="flex justify-between items-center text-sm">
              ...
              <span className="text-xl font-black text-[#00c875] font-mono">{calcVolume.toLocaleString()} / mo</span>
            </div>
            ...
            <input
              ...
              className="w-full accent-[#00c875] cursor-pointer"
            />

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-slate-200 dark:border-slate-800 text-xs">
              <div className="p-4 rounded-2xl bg-slate-100 dark:bg-[#091624] border border-slate-200 dark:border-slate-800">
              ...
              <div className="p-4 rounded-2xl bg-[#00c875]/10 border border-[#00c875]/30">
                <span className="text-[#00c875] block font-bold">Free Meta Allowance Included</span>
            ...
            return (
              ...
              >
                <button
                  ...
                  className="w-full p-5 text-left font-bold text-slate-900 dark:text-white text-sm sm:text-base flex justify-between items-center gap-4 hover:text-[#00c875] transition-colors"
                >
                  <span>{faq.q}</span>
                  <ChevronDown className={`w-5 h-5 text-[#00c875] shrink-0 transition-transform ${isOpen ? 'rotate-180' : ''}`} />
                </button>
                {isOpen && (
